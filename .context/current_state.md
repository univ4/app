...
### Calculators (21개, `src/lib/calculators/`)

- `analyzeSubjectAdvantage.ts`, **`calcSubjectAdvantage.ts`** (P1-11 정시 반영비 유불리), **`calcScienceComboSimulator.ts`** (P3-4 과탐 조합·가산), **`calcEarlyRoadmap.ts`** (P3-3 고1·고2 조기 설계 로드맵), **`calcNsuStrategy.ts`** (P2-8a 재수·N수 전략 참고), **`calcAdmissionTrend.ts`** (P2-9 연도별 입결 추이), `calculateAdmissionProbability.ts`, `calculateSuneungScore.ts`, `calculateSusiGPA.ts`, `calculateZScore.ts`, **`calcSchoolLevel.ts`** (P1-2 Z·고교 수준 참고)
...
- **P3-3** `src/app/dashboard/early-roadmap/page.tsx` — 조기 설계 로드맵(`RoadmapSetup`, `RoadmapTimeline`; `POST /api/early-roadmap`)
...
- **`api/early-roadmap/route.ts`** (P3-3)
...
- Calculators × 20+, API routes × 8+, lib/chat × 1, calendar 통합 × 1, explore 헬퍼 × 1 — `npm test` 스위트 수는 실행 시 확인
...
- **결과:** **400 tests PASS** / **70 suites** / FAIL 0 (`npm test`, P3-3 `early-roadmap`·P2-11 `scores-parse-image`·NEIS 매핑 테스트 포함)
