...
### Calculators (20개, `src/lib/calculators/`)

- `analyzeSubjectAdvantage.ts`, **`calcSubjectAdvantage.ts`** (P1-11 정시 반영비 유불리), **`calcScienceComboSimulator.ts`** (P3-4 과탐 조합·가산), **`calcEarlyRoadmap.ts`** (P3-3 고1·고2 조기 설계 로드맵), **`calcAdmissionTrend.ts`** (P2-9 연도별 입결 추이), `calculateAdmissionProbability.ts`, `calculateSuneungScore.ts`, `calculateSusiGPA.ts`, `calculateZScore.ts`, **`calcSchoolLevel.ts`** (P1-2 Z·고교 수준 참고)
