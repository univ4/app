...
import {
  applyExploreFilters,
  buildSusiRuleMap,
  parseAdmissionTypeListParam,
  parseSignalListParam,
} from "@/lib/explore/applyExploreFilters";
...
const triStateSchema = z.enum(["true", "false", "all"]);
...
const querySchema = z.object({
  ...
  region: z.enum(["서울", "수도권", "지방", "all"]).optional(),
  suneungMin: triStateSchema.optional(),
  noInterview: triStateSchema.optional(),
});
...
export async function GET(request: NextRequest) {
  ...
  const parsed = querySchema.safeParse({
    ...
    admissionType: url.searchParams.get("admissionType") ?? undefined,
    signal: url.searchParams.get("signal") ?? undefined,
    region: url.searchParams.get("region") ?? undefined,
    suneungMin: url.searchParams.get("suneungMin") ?? undefined,
    noInterview: url.searchParams.get("noInterview") ?? undefined,
  });
  ...
  if (targetStudentId !== user.id) {
    return NextResponse.json(
      {
        data: null,
        error: { code: "FORBIDDEN", message: "다른 학생 탐색 결과는 조회할 수 없습니다." },
      },
      { status: 403 },
    );
  }
  ...
  const admissionTypes = parseAdmissionTypeListParam(parsed.data.admissionType ?? "all");
  const signals = parseSignalListParam(parsed.data.signal ?? "all");
  const region = parsed.data.region ?? "all";
  const suneungMin = parsed.data.suneungMin ?? "all";
  const noInterview = parsed.data.noInterview ?? "all";

  if (
    (parsed.data.admissionType != null &&
      parsed.data.admissionType !== "" &&
      parsed.data.admissionType !== "all" &&
      admissionTypes == null) ||
    (parsed.data.signal != null &&
      parsed.data.signal !== "" &&
      parsed.data.signal !== "all" &&
      signals == null)
  ) {
    return NextResponse.json(
      {
        data: null,
        error: { code: "VALIDATION_ERROR", message: "admissionType·signal 값이 올바르지 않습니다." },
      },
      { status: 422 },
    );
  }
  ...
  const [
    ...
    supabase
      .from("susi_gpa_rules")
      .select(
        "university_name, admission_type, include_subjects, career_choice_conversion, suneung_minimum, interview_required",
      )
      .eq("admission_year", admissionYear)
  ...
  const built = buildAdmissionSignalRows({
  ...
  const ruleMap = buildSusiRuleMap(
    (susiRows ?? []).map((r) => ({
      university_name: r.university_name as string,
      admission_type: r.admission_type as string,
      suneung_minimum: r.suneung_minimum,
      interview_required: (r.interview_required as boolean | null) ?? null,
    })),
  );

  const items = applyExploreFilters(built, ruleMap, {
    admissionTypes,
    signals,
    region,
    suneungMin,
