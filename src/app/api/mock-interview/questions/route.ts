...
import {
  ...
  buildMockInterviewQuestionsSystemPrompt,
  buildMockInterviewQuestionsUserMessage,
  buildMockInterviewRecordContext,
  sseStreamMockInterviewNoChunks,
  sseStreamMockInterviewNoGuidelines,
...
export async function POST(request: Request) {
  ...
  const { targetUniv, interviewType } = parsed.data;
  ...
  if (chunks.length === 0) {
    const out = sseStreamMockInterviewNoChunks(
      "생활기록부 청크(student_record_chunks)가 없습니다. 생활기록부 입력 후 임베딩 적재(embed_student_record)를 진행한 뒤 다시 시도해 주세요.",
    );
  ...
  if (guidelineChunks.length === 0) {
    const out = sseStreamMockInterviewNoGuidelines(
      `목표 대학「${targetUniv}」에 해당하는 전형계획 청크를 유사도 기준으로 찾지 못했습니다. guideline_chunks 적재·CHAT_SIMILARITY_THRESHOLD를 확인해 주세요.`,
    );
    return new Response(out, {
      headers: {
        "Content-Type": "text/event-stream; charset=utf-8",
  ...
  const recordContext = buildMockInterviewRecordContext(chunks);
  const guidelineContext = buildReferenceContext(guidelineChunks);

  const system = buildMockInterviewQuestionsSystemPrompt(
    recordContext,
    guidelineContext,
    interviewType,
  );
  const userContent = buildMockInterviewQuestionsUserMessage(targetUniv);
