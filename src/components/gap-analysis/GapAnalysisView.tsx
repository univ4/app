...
import { GapAnalysisResult } from "@/components/gap-analysis/GapAnalysisResult";
import type { GapAnalysisDonePayload } from "@/types/gapAnalysis";
...
function flushSseBuffer(
  ...
  onDone: (payload: GapAnalysisDonePayload) => void,
): string {
  ...
  for (const part of parts) {
    ...
    try {
      if (parsed.event === "chunk") {
        ...
        const j = JSON.parse(parsed.data) as GapAnalysisDonePayload;
        onDone(j);
      }
    } catch {
      /* skip malformed */
...
export function GapAnalysisView() {
  ...
  const [donePayload, setDonePayload] = useState<GapAnalysisDonePayload | null>(null);
  ...
  const runGapAnalysis = useCallback(async () => {
    ...
    let res: Response;
    try {
      res = await fetch("/api/student-record/gap-analysis", {
        method: "POST",
    ...
    let carry = "";
    const streamResult: { done: GapAnalysisDonePayload | null } = { done: null };

    const appendChunk = (chunk: string) => {
      setStreamingText((prev) => prev + chunk);
    };

    const onDone = (p: GapAnalysisDonePayload) => {
      streamResult.done = p;
    };
    ...
    const drainTail = (tail: string) => {
      ...
      try {
        if (parsed.event === "chunk") {
          ...
          onDone(JSON.parse(parsed.data) as GapAnalysisDonePayload);
        }
      } catch {
        /* ignore */
      }
    ...
    setLoading(false);
  }, [loading, remainingWeeks, targetUniv]);
  ...
  return (
    <div className="space-y-6">
      <GapAnalysisForm
        ...
        onRemainingWeeksChange={setRemainingWeeks}
        loading={loading}
        onStart={() => void runGapAnalysis()}
      />
