    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-3 rounded-lg border border-border bg-white p-4 shadow-sm">
        <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-end sm:justify-between">
          <div>
            <h2 className="text-lg font-semibold text-foreground">합격 가능성 신호등</h2>
        ...
        {meta ? (
          <p className="text-muted-foreground text-xs">
            스캔 {meta.row_count}행 · 고유 대학 약 {meta.unique_universities}개 · 응답{" "}
            {meta.duration_ms}ms
            ...
            {filtered.length === 0 ? (
              <TableRow>
                <TableCell colSpan={6} className="text-muted-foreground py-10 text-center text-sm">
                  {loading
                    ? "불러오는 중입니다…"
                    : "조건에 맞는 행이 없습니다. 전체 대학 스캔을 실행하거나 필터를 바꿔 보세요."}
                </TableCell>
              </TableRow>
            ) : (
              filtered.map((r) => (
