...
### P2-8a. 재수생(N수) 전략 특화 *(Track 1 참고 뷰)*

**설명**
- 재수 연차·전년 대비 수능 점수 추이·내신(선택)을 입력하면 정시·수시 균형에 대한 **참고용** 권고 문구를 표시한다.
- 학생부 3-2 미반영·N수 불리 전형 등 수시 시 주의사항을 목록으로 안내한다.
- `admission_records`·신호등·전국 탐색과 병행해 지원 범위를 좁히는 데 활용한다.

**트랙 분류**
- 규칙 기반 문구 생성 → **Track 1** (`calcNsuStrategy`)

**User Story**
- 나는 재수·N수 지원자로서 내신이 불리한 상황에서 정시와 수시를 어떻게 배분할지 방향을 잡고 싶다.

**Acceptance Criteria**
1. 재수 연차(1~4+)·전년·현재 수능 점수·내신·목표 전형(정시/수시/병행) 입력 후 권고 전략이 표시된다.
2. 점수 향상·하락·삼수 이상·내신 미입력 등 조합별로 안내 문구가 달라진다.
3. 수시 주의사항에 학생부 반영 제한·N수 불리 전형 경고가 포함된다.
