...
- **P3: 5개** <!-- v2: 정시 배치표는 P2-12 통합; 카카오톡 알림(P3-2) 제거; +P3-6 -->
...
### P2-8a. 재수생(N수) 전략 특화 <!-- v2 -->

**설명**
- 재수 연차·전년 대비 수능 점수·내신(선택)을 입력하면 정시·수시 균형에 대한 참고 권고 문구를 표시한다.
- 학생부 3-2 미반영·N수 불리 전형 등 수시 주의사항을 안내한다.

**트랙 분류**: Track 1 (`calcNsuStrategy`) · UI `/dashboard/nsu-strategy` · `POST /api/nsu-strategy`

**Acceptance Criteria**
1. 재수 연차·점수·내신·목표 전형 입력 후 권고 전략·정시 유리 여부·주의사항이 표시된다.
2. 점수 향상·하락·삼수 이상·내신 미입력 등 조합별 안내가 달라진다.
...
### P3-3. 고1/고2 선행 관리 · 조기 설계 로드맵 <!-- v2: 구 P3-4 -->
**설명**
- 학년·학기·목표 대학 수준·계열을 입력하면 **고1~고2 네 학기** 기준 내신 목표·우선순위·권장 활동·마일스톤을 규칙 기반으로 제시한다(Track 1: `calcEarlyRoadmap`).
- 합격·컷을 보장하지 않으며, 학교 교육과정·상담 지침을 우선한다.

**화면**
- `/dashboard/early-roadmap` — 타임라인·현재 학기 하이라이트·우선순위 체크리스트

**API**
- `POST /api/early-roadmap` — 인증 필요; 요청 `{ currentGrade, currentSemester, targetUnivType, targetDept }`

**Acceptance Criteria**
1. 고1·고2 각 1·2학기별 단계(기간·내신 목표·우선순위·활동) 표시
2. 목표 대학 수준(top/mid/local)에 따라 내신 목표 문구 강화·기본·완화
3. 목표 계열(이공/인문·사회/예체능)에 따른 권장 활동 보강
4. 현재 학년·학기에 해당하는 학기 카드 시각적 강조
