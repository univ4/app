...
## 1b-3. `calcEarlyRoadmap` (P3-3)

구현·테스트: `src/lib/calculators/calcEarlyRoadmap.ts`, `src/__tests__/calculators/calcEarlyRoadmap.test.ts`

| ID | 시나리오 | 기대 |
|---|---|---|
| CER-01 | 고1 1학기 + `top` | 고1 1학기 `gpaTarget`에 강화 문구(예: 1.7등급)·`summary`에 상향 |
| CER-02 | 고1 2학기 + `mid` | 기본 내신 문구(2등급 이내·1~2등급 유지)·`summary`에 기본값 |
| CER-03 | 고2 1학기 + `local` | 고2 1학기 `gpaTarget`에 완화·지역 문구·`summary`에 하향 |
| CER-04 | 고2 2학기 + `science` | 고2 2학기 활동에 이공·수능/과탐 관련 문구 |

**API**: `POST /api/early-roadmap` — `src/__tests__/api/early-roadmap.route.test.ts` (비인증 401, 인증 200·`phases`·`summary`)

---

## 1b-4. `calcNsuStrategy` (P2-8a)

구현·테스트: `src/lib/calculators/calcNsuStrategy.ts`, `src/__tests__/calculators/calcNsuStrategy.test.ts`

| ID | 시나리오 | 기대 |
|---|---|---|
| CNS-01 | 재수 1년·전년 대비 점수 상승 | `jeongsiAdvantage === true`, `scoreImprovement` 양수, 권고에 상승 문구 |
| CNS-02 | 재수 1년·점수 하락 | `jeongsiAdvantage === false`, 수시 분산·정시 불리 톤 |
| CNS-03 | 삼수 이상 | 정시 집중 톤, `jeongsiAdvantage === true` |
| CNS-04 | 내신 미입력 | `warnings`에 내신·정시 우선 안내 |
| CNS-05 | 삼수 이상·점수 하락 | 하락 경고와 정시 집중 병행 |

**API**: `POST /api/nsu-strategy` — `src/__tests__/api/nsu-strategy.route.test.ts` (비인증 401, 인증 200·`data.strategy`)
